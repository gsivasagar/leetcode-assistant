...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, index, onLanguageChange }) => {
    ...
    if (isModel && isStructuredAnalysis(message.content)) {
        ...
        return (
            <div className="flex items-start gap-4 animate-slide-up">
                <div className="flex-shrink-0 h-10 w-10 rounded-full flex items-center justify-center bg-cyan-100 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400 shadow-inner border border-cyan-200/50 dark:border-cyan-700/50">
                    <BotIcon className="w-6 h-6" />
                </div>
                <div className="relative w-full rounded-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                    <AccordionSection
                        title="Hints"
                                ...
                                <div key={idx}>
                                    {!revealedHints.has(idx) ? (
                                        <button
                                            ...
                                            className="px-4 py-2 text-xs font-semibold text-cyan-700 dark:text-cyan-300 bg-cyan-50 dark:bg-cyan-900/50 rounded-lg hover:bg-cyan-100 dark:hover:bg-cyan-900 shadow-sm border border-cyan-100 dark:border-cyan-800 transition-colors"
                                        >
                                            Reveal Hint {idx + 1}
                                        </button>
                                    ) : (
    ...
    return (
        <div className={`flex items-start gap-4 animate-slide-up ${!isModel ? 'flex-row-reverse' : ''}`}>
            {isModel && (
                <div className="flex-shrink-0 h-10 w-10 rounded-full flex items-center justify-center bg-cyan-100 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400 shadow-inner border border-cyan-200/50 dark:border-cyan-700/50">
                    <BotIcon className="w-6 h-6" />
                </div>
            )}
            <div className={`relative max-w-2xl rounded-2xl px-5 py-3.5 shadow-sm ${isModel ? 'bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border border-gray-200 dark:border-gray-700' : 'bg-gradient-to-br from-cyan-600 to-blue-600 text-white shadow-cyan-500/20'}`}>
                <div>
            ...
            {!isModel && (
                <div className="flex-shrink-0 h-10 w-10 rounded-full flex items-center justify-center bg-gradient-to-br from-cyan-500 to-blue-500 text-white shadow-inner">
